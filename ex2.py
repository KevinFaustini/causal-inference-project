...
Z = MatrixZFct(MarketData, Klist2,2)
print(len(Yt))
print(len(Z))
#print(np.concatenate((Yt,Z),axis=0))
...
#     for t in range(6):
#         Yt=VectorFct(MarketData,y,t+1)[0]
#         Z=MatrixZFct(MarketData,UsedList,t)[0]
#         X=VectorFct(MarketData,x,t)[0]
#         Y=VectorFct(MarketData,y,t)[0]
#

        # dimensions = (190,(t+1)+(t+1)*10)
        # EmptyMatrix=np.zeros(dimensions)
        # for r in range(190):
        #     for c in range(t):
        #        EmptyMatrix.append(Yt[r][c])
        # print(EmptyMatrix)
        ...
        # temp+=log((Elmnt1*Elmnt2)/(Elmnt3*Elmnt4))
    #DI=temp*1/2
    #print(DI)
    #DIList.append(DI)
